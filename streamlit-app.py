        ...
        try:
            ...
            if viz_type == 'Feature Distribution':
                try:
                    ...
                    # Make sure Revenue is boolean type
                    df['Revenue'] = df['Revenue'].astype(bool)
                    
                    # Create two separate distribution plots for better clarity
                    fig, ax = plt.subplots(figsize=(10, 6))
                    
                    # Try KDE plot first which works better with categorical hue
                    try:
                        sns.histplot(data=df, x=feature_for_viz, kde=True, hue='Revenue', 
                                    palette='viridis', alpha=0.6, element="step", common_norm=False)
                        plt.title(f'Distribution of {feature_for_viz} by Revenue')
                        st.pyplot(fig)
                    except Exception as e:
                        st.warning(f"Could not create combined histplot: {e}")
                        
                        # Fallback to separate plots
                        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8), sharex=True)
                        
                        # Plot for Revenue=True
                        sns.histplot(df[df['Revenue']==True][feature_for_viz], 
                                    kde=True, ax=ax1, color='green', label='Revenue=True')
                        ax1.set_title(f'Distribution of {feature_for_viz} (Revenue=True)')
                        ax1.legend()
                        
                        # Plot for Revenue=False
                        sns.histplot(df[df['Revenue']==False][feature_for_viz], 
                                    kde=True, ax=ax2, color='blue', label='Revenue=False')
                        ax2.set_title(f'Distribution of {feature_for_viz} (Revenue=False)')
                        ax2.legend()
                    ...
                    # Create a box plot as an alternative view
                    fig, ax = plt.subplots(figsize=(10, 6))
                    sns.boxplot(x='Revenue', y=feature_for_viz, data=df, palette='viridis')
                    plt.title(f'Box Plot of {feature_for_viz} by Revenue')
                    st.pyplot(fig)
                    
                except Exception as e:
                    st.error(f"Error in Feature Distribution visualization: {e}")
                    st.write("Try selecting a different feature or visualization type.")
                ...
                try:
                    ...
                    corr_df = df_corr.select_dtypes(include=['float64', 'int64']).corr()
                    
                    # Plot correlation matrix
                    fig, ax = plt.subplots(figsize=(12, 10))
                    mask = np.triu(np.ones_like(corr_df, dtype=bool))  # Create mask for upper triangle
                    sns.heatmap(corr_df, annot=True, cmap='coolwarm', fmt='.2f', 
                                linewidths=0.5, mask=mask)
                    plt.title('Correlation Matrix')
                    st.pyplot(fig)
                    
                    # Show the most important correlations with Revenue
                    if 'Revenue' in corr_df.columns:
                        st.subheader('Top Correlations with Revenue:')
                        revenue_corr = corr_df['Revenue'].sort_values(ascending=False)
                        # Remove self-correlation
                        revenue_corr = revenue_corr[revenue_corr.index != 'Revenue']
                        st.write(revenue_corr)
                        
                        # Plot top correlations with Revenue
                        fig, ax = plt.subplots(figsize=(10, 6))
                        sns.barplot(x=revenue_corr.values, y=revenue_corr.index, palette='viridis')
                        plt.title('Feature Correlation with Revenue')
                        plt.xlabel('Correlation Coefficient')
                        st.pyplot(fig)
                        
                except Exception as e:
                    st.error(f"Error in Correlation Matrix visualization: {e}")
                    st.write("Check if your dataset contains numeric columns for correlation analysis.")
