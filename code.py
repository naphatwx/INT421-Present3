...
try:
    ...
    st.error("Models not found. Please make sure 'random_forest_model.pkl', 'xgboost_model.pkl', and 'scaler.pkl' are in the same directory.")
    models_loaded = False
...
def preprocess_input(input_df, is_single=True):
    ...
    if 'VisitorType' in df_processed.columns:
        df_processed = pd.get_dummies(df_processed, columns=['VisitorType'], drop_first=True)
    
    # If it's a single input, make sure all expected columns are present
    if is_single:
        # Get the expected column names from the models
        sample_df = pd.read_csv('online_shoppers_intention.csv', nrows=1)
        sample_df = sample_df.drop('Revenue', axis=1)
        sample_df = pd.get_dummies(sample_df, columns=['Month', 'VisitorType'], drop_first=True)
...
if models_loaded:
    ...
    if st.session_state.tab_selected == 0:
        ...
        if st.button('Predict'):
            ...
            # Get feature names (using the preprocessed input columns)
            sample_df = pd.read_csv('online_shoppers_intention.csv', nrows=1)
            sample_df = sample_df.drop('Revenue', axis=1)
            sample_df = pd.get_dummies(sample_df, columns=['Month', 'VisitorType'], drop_first=True)
